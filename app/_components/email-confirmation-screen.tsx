...
function SuwaveLogo() {
  return (
    <Link aria-label="Suwave logista" className="inline-flex items-end text-inherit no-underline" href="/">
      <span className="text-[46px] font-black leading-none tracking-normal text-ink sm:text-[58px]">
        SU
...
function EmailIllustration() {
  return (
    <div aria-hidden="true" className="relative mx-auto mt-16 grid h-[190px] w-[230px] place-items-center">
      ...
      <span className="absolute bottom-8 h-[78px] w-[148px] rounded-[6px] border-[3px] border-[#8b8f96] bg-surface">
      ...
      <span className="absolute top-10 grid h-[96px] w-[96px] place-items-center rounded-[12px] bg-brand text-white shadow-[0_16px_24px_rgba(246,189,20,.24)] before:absolute before:bottom-[-19px] before:h-10 before:w-10 before:rotate-45 before:rounded-[5px] before:bg-brand">
        <span className="relative z-10 grid h-[58px] w-[58px] place-items-center rounded-full bg-surface text-brand">
          <Check className="h-9 w-9" strokeWidth={3.2} />
        </span>
      </span>
      <span className="absolute right-3 top-12 h-9 w-10 border-r-[4px] border-t-[4px] border-brand" />
      <span className="absolute right-0 top-30 h-1 w-6 rotate-[14deg] rounded-full bg-brand" />
    </div>
  );
}
...
export function EmailConfirmationScreen() {
  ...
  return (
    <main className="min-h-screen bg-surface-3 px-4 py-4 text-ink sm:px-7 sm:py-6">
      <section className="mx-auto min-h-[calc(100vh-48px)] w-full max-w-[1080px] rounded-[16px] bg-surface px-5 py-8 shadow-[0_22px_70px_rgba(0,0,0,.08)] sm:px-12 lg:px-[92px] lg:py-12">
        <div className="mx-auto flex min-h-[calc(100vh-144px)] max-w-[760px] flex-col">
          <header className="grid justify-items-center">
            <SuwaveLogo />
            <EmailIllustration />
            <h1 className="mt-8 text-center text-[34px] font-black leading-tight tracking-normal text-ink sm:text-[48px]">
              Confirme seu e-mail
            </h1>
            <p className="mt-6 max-w-[620px] text-center text-[18px] font-semibold leading-8 text-ink-3">
          ...
          <section className="mt-7 grid justify-items-center">
            <div className="flex items-center gap-5 text-[20px] font-black text-ink">
              <Mail aria-hidden="true" className="h-8 w-8 text-brand" strokeWidth={2.3} />
              <span>{email}</span>
            </div>

            <p className="mt-12 text-center text-[20px] font-black text-ink">Digite o código de 4 dígitos</p>
            <div className="mt-6 flex justify-center gap-5 sm:gap-7">
              {code.map((digit, index) => (
                <input
                  aria-label={`Dígito ${index + 1} do código`}
                  className="h-[76px] w-[76px] rounded-[13px] border-2 border-line-strong bg-surface text-center text-[34px] font-black text-ink caret-ink outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/20 sm:h-[88px] sm:w-[88px]"
                  inputMode="numeric"
                  key={index}
                  maxLength={1}
            ...
            <p className="mt-7 text-center text-[17px] font-semibold text-[#7a7d86]">
              O código expira em <strong className="font-black text-brand">{formattedTime}</strong>
            </p>
          </section>

          <section className="mx-auto mt-16 flex w-full max-w-[660px] gap-5 rounded-[13px] border border-[#e0e0e0] bg-surface-2 px-7 py-6 shadow-[0_8px_24px_rgba(0,0,0,.03)]">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full border-2 border-brand text-brand">
              <Info aria-hidden="true" className="h-7 w-7" strokeWidth={2.4} />
            </span>
            <div>
              <h2 className="text-[20px] font-black leading-tight text-ink">Não recebeu o código?</h2>
          ...
          <footer className="mt-auto flex flex-col-reverse gap-3 pt-12 sm:flex-row sm:items-center sm:justify-between">
            <Link
              className="inline-flex h-[66px] items-center justify-center gap-4 rounded-[9px] border border-line-strong bg-surface px-9 text-[20px] font-black text-ink no-underline transition hover:bg-surface-2"
              href="/register"
            >
            ...
            <button
              className="inline-flex h-[66px] items-center justify-center gap-8 rounded-[9px] bg-brand px-12 text-[20px] font-black text-ink transition hover:bg-brand-strong"
